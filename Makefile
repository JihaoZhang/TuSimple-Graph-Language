...
clean :
	ocamlbuild -clean
	rm -rf testall.log *.diff tusimple scanner.ml parser.ml parser.mli
	rm -rf printbig
	rm -rf *.cmx *.cmi *.cmo *.cmx *.o *.s *.ll *.out *.exe

# More detailed: build using ocamlc/ocamlopt + ocamlfind to locate LLVM

OBJS = ast.cmx codegen.cmx parser.cmx scanner.cmx semant.cmx tusimple.cmx

tusimple : $(OBJS)
	ocamlfind ocamlopt -linkpkg -package llvm -package llvm.analysis $(OBJS) -o tusimple
...
codegen.cmo : ast.cmo
codegen.cmx : ast.cmx
tusimple.cmo : semant.cmo scanner.cmo parser.cmi codegen.cmo ast.cmo
tusimple.cmx : semant.cmx scanner.cmx parser.cmx codegen.cmx ast.cmx
...
TARFILES = ast.ml codegen.ml Makefile tusimple.ml parser.mly README scanner.mll \
	semant.ml testall.sh $(TESTFILES:%=tests/%) printbig.c arcade-font.pbm \
	font2c

tusimple-llvm.tar.gz : $(TARFILES)
	cd .. && tar czf tusimple-llvm/tusimple-llvm.tar.gz \
		$(TARFILES:%=tusimple-llvm/%)
